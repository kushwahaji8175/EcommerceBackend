...
public class AuthController {
    ...
    public ResponseEntity<String> login(@RequestBody LoginRequest loginRequest){
        ...
        if(auth.isAuthenticated()) {
            if(userDetails.isEmailConfirmation()){
            final String jwt = jwtService.generateToken(userDetails.getEmail());
            return ResponseEntity.ok(jwt);}
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("user does not confirm his email yet");
        }
